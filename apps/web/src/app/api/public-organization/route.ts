...
export async function GET(){
  try{
    ...
    const response=await fetch(new URL("/api/v1/organization/public",config.base),{
  ...
  }catch{return Response.json({message:"API no disponible"},{status:503,headers:{"Cache-Control":"no-store"}});}
}
