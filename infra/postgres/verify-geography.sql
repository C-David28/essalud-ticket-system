...
SELECT pg_temp.assert_true((SELECT count(*)=5 FROM information_schema.columns WHERE table_schema='app'
  AND table_name='centros_asistenciales' AND column_name IN ('latitude','longitude','location_source','location_accuracy','address')),
  'catalogo de sedes contiene coordenadas, direccion y precision');
...
INSERT INTO app.centros_asistenciales(red_asistencial_id,centro_asistencial_id,codigo,nombre,tipo,address,latitude,longitude,location_source,location_accuracy) VALUES
 ('4a000000-0000-4000-8000-000000000001','4a100000-0000-4000-8000-000000000001','GEO_SEDE_A','Sede ficticia A','HOSPITAL','Direccion ficticia A',-10.686800,-76.256500,'CONFIGURED','REFERENCE'),
 ('4b000000-0000-4000-8000-000000000001','4b100000-0000-4000-8000-000000000001','GEO_SEDE_B','Sede ficticia B','CAP','Direccion ficticia B',-10.575500,-75.405300,'NETWORK','DEMO');
...
SELECT pg_temp.assert_true((SELECT latitude IS NULL AND longitude IS NULL AND location_source IS NULL AND location_accuracy IS NULL
...
SELECT pg_temp.expect_error($q$INSERT INTO app.centros_asistenciales(red_asistencial_id,centro_asistencial_id,codigo,nombre,tipo,latitude,longitude,location_source,location_accuracy)
 VALUES('4a000000-0000-4000-8000-000000000001','4a100000-0000-4000-8000-000000000003','BAD_LAT','Invalida','OTRO',91,-76,'CONFIGURED','REFERENCE')$q$,'23514','latitud fuera de rango rechazada');
...
SELECT pg_temp.expect_error($q$INSERT INTO app.centros_asistenciales(red_asistencial_id,centro_asistencial_id,codigo,nombre,tipo,latitude,longitude,location_source,location_accuracy)
 VALUES('4a000000-0000-4000-8000-000000000001','4a100000-0000-4000-8000-000000000005','BAD_SOURCE','Origen invalido','OTRO',-10,-76,'DEVICE','REFERENCE')$q$,'23514','origen no controlado rechazado');
SELECT pg_temp.assert_true((SELECT new_values->>'latitude'='-10.686800' AND new_values->>'location_source'='CONFIGURED'
  AND new_values->>'location_accuracy'='REFERENCE' AND new_values->>'address'='Direccion ficticia A'
...
UPDATE app.centros_asistenciales SET latitude=-10.700000,longitude=-76.260000,location_source='GEOCODED',location_accuracy='REFERENCE'
